...
from ShasaBot import DEV_USERS, EVENT_LOGS, OWNER_ID
from ShasaBot import mongodb as db
...
@tbot.on(events.NewMessage(pattern="^/pban (.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@tbot.on(events.NewMessage(pattern="^/unpban (.*)"))
async def _(event):
    if event.fwd_from:
        return
    if event.sender_id in DEV_USERS:
...
async def join_ban(event):
    if event.chat_id == int(EVENT_LOGS):
        return
    if event.chat_id == int(EVENT_LOGS):
        return
    user = event.user_id
